...
for (const line of lines) {
  ...
  for (let i = solutionLine.intervalSteps.length - 2; i >= 0; i--) {
    ...
    let lastValue = currentIntervalStep[0];

    let nextIntervalStep = solutionLine.intervalSteps[i + 1];
    let nextIntervalStepLastValue = nextIntervalStep[0];

    solutionLine.intervalSteps[i].unshift(
      lastValue - nextIntervalStepLastValue,
    );
  }

  solutionLine.addedNumber = solutionLine.intervalSteps[0][0];

  solution.lines.push(solutionLine);
}

console.dir(solution, { depth: null });
